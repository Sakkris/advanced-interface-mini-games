...
public class WebSocketServerManager : MonoBehaviour
{
    ...
    public event EventHandler OnVoiceTextChange;
    ...
    private void Start()
    {
        ...
        // Create services for both hand tracking and voice recog
        wss.AddWebSocketService<HandRecognitionSocketBehavior>("/HandTracking");
        wss.AddWebSocketService<VoiceRecognitionSocketBehavior>("/VoiceRecognition");
        wss.Start();
    ...
    {
        if(!String.Equals(prevVoiceText, voiceText))
        {
            prevVoiceText = voiceText;
            subtitle.text = voiceText;
            
            OnVoiceTextChange?.Invoke(this, EventArgs.Empty);
        }
    }

    public void ParseHandData(Vector2 fingerPositionVector, string gestureString, bool rightHandDetected, bool leftHandDetected)
    {
...
public class HandRecognitionSocketBehavior : WebSocketBehavior
{
    protected override void OnMessage(MessageEventArgs e)
    {
        ...
        Debug.Log($"Received hand data: {data}");

        // Process hand tracking data
        WebSocketServerManager.Instance.ParseHandData(fingerPositionVector, gestureString, rightHandDetected, leftHandDetected);
    }
}

public class VoiceRecognitionSocketBehavior : WebSocketBehavior
{
    protected override void OnMessage(MessageEventArgs e)
    {
        JObject data = JObject.Parse(e.Data);

        if (data["text"] != null && !String.Equals(data["text"].ToString(), ""))
        {
            string voiceText = data["text"].ToString();
            Debug.Log($"Received voice data: {voiceText}");

            // Process voice recognition data
            WebSocketServerManager.Instance.ParseVoiceData(voiceText);
        }
    }
}
